# GOAL: Voynich Research Dossier - naibbe_cipher. Mother May I.

# INDEPENDENT AUDIT — "Naibbe cipher" Voynich claim

  ## 0. TOOLING NOTE (affects what I could verify)

  `WebFetch` is blocked in this repo by the `NO-WEBFETCH GUARDIAN` PreToolUse hook, which redirects to `python tools/run_raven_scout.py`. **That file does not exist** (`tools/run_raven_scout.py` — No such file or directory). I worked around it by calling `core.huginn.HuginnObserver.observe_url` directly from a scratchpad script.

  **Repo bug found (reported, NOT fixed — Prime Rule / P39):** `c:\flask-structural-api\core\huginn.py` calls `requests.get(...)` at line ~127 but **never imports `requests`**. Every `observe_url()` call dies with `NameError: name 'requests' is not defined`. This means the RAVEN URL path is dead on the current working tree. I injected `requests` into the module namespace at runtime from my own scratchpad only. Two consequences for the Captain: (a) the no-WebFetch hook currently points at a non-existent tool, so the mandated fetch path is entirely broken; (b) `core/huginn.py` needs one import line plus a P44 sentinel.

  **One source I could NOT retrieve:** the Cryptologia article full text at `tandfonline.com` returns **HTTP 403** (Cloudflare) on every URL form I tried (`/doi/full/`, `/doi/pdf/?needAccess=true`), across RAVEN's User-Agent rotation. Unpaywall confirms it is OA (`is_oa: true`, `oa_status: hybrid`, publisher-hosted only, `has_repository_copy: false`, `url_for_pdf: null`) — i.e. **there is no repository mirror**. So I have NOT read the main paper body. I read the author's 48-page Supplementary Materials PDF from Zenodo instead, plus the author's own repo/site, plus two independent papers that re-implemented the cipher from its published tables.
  ...
  **It is a generative model that produces Voynich-LIKE text from Latin/Italian. It does NOT claim to read the actual manuscript. Not one word of the Voynich Manuscript is translated anywhere in this work.**
  ...
  - **EVA alphabet:** Naibbe ciphertexts achieve character entropy **3.86 ± 0.01** and conditional character entropy **2.00 ± 0.01** at token level. **REPORTED-ONLY** — this came back in a WebSearch summary of the tandfonline page I could not fetch. I did **not** read it in a primary source. Treat as unconfirmed.
  - **Cuva alphabet:** Naibbe **3.92 ± 0.01** / **2.32 ± 0.01**, vs Voynich B **3.90** / **2.25**. **REPORTED-ONLY**, same caveat.
  ...
  **The work is purely text-statistical. It does not analyse, interpret, or make any claim about the content of the botanical, astronomical, balneological, or pharmaceutical illustrations.** It has nothing to say about what any section depicts.
  ...
  ## 8. OVERLAP WITH THE ORACLE COLLECTIVE FRAMEWORK (2026-03-20)
  ...
  **FETCH FAILED — honest empty result**
  - https://www.tandfonline.com/doi/full/10.1080/01611194.2025.2566408 — **HTTP 403** (Cloudflare), all URL forms, all User-Agents. **I have not read the main paper body.** The two entropy figures in §4 marked REPORTED-ONLY (3.86/2.00 EVA; 3.92/2.32 vs 3.90/2.25 Cuva) derive from a WebSearch summary of this page and should be independently confirmed before being cited as the paper's numbers.
  - https://zenodo.org/api/records/16415087/files — returned empty
